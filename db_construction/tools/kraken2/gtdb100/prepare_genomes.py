...
def get_ref(genomes_info, strain_taxid_file):
    ...
    if strain_taxid_file:
        ...
        merge_df = pd.merge(reference_genomes_df, strain_taxid_df, on="genome_ID")
...
def fa_format_process(args):
    ...
    output_file_path = Path(genomes_output_dir) / Path(reference_strain[1]).name.replace(".gz", "")
    if not output_file_path.exists():  
        with open_file(reference_strain[1]) as input_file, open(output_file_path, 'w') as output_file:
