...
echo "###########################################################################################"
echo "Running sylph..."
mkdir -p $wd/sylph/$profile_level && cd $wd/sylph/$profile_level
mkdir -p $wd/sylph/$profile_level/$dataset/$samplesID && cd $wd/sylph/$profile_level/$dataset/$samplesID
...
if [ ! -f "evaluation_report.txt" ]; then
    ...
    if [ $profile_level == "species_level" ]; then
        ...
        python $scripts_dir/species_metrics_eval.py -i - -t sylph -rt $read_type -s $samplesID -dt $data_type -pa sylph_abundance.txt -ta $true_abund -m $camisim_reads_mapping_path
        python $scripts_dir/species_metrics_eval.py -i - -t sylph -rt $read_type -s $samplesID -dt $data_type -pa sylph_abundance.txt -ta $true_abund -m $camisim_reads_mapping_path -e 0
    elif [ $profile_level == "strain_level" ]; then
        python $scripts_dir/sylph_convert.py result.tsv $seq2tax strain
        python $scripts_dir/strain_evaluation.py sylph_abundance.txt sylph $data_type $true_abund $database_genomes_info > evaluation_report.txt
    fi
fi
...
if [ ! -f "evaluation_report.txt" ]; then
    ...
    if [ $profile_level == "species_level" ]; then
        python $scripts_dir/sylph_convert.py result.tsv $seq2tax species
        python $scripts_dir/species_metrics_eval.py -i - -t sylph -rt $read_type -s $samplesID -dt $data_type -pa sylph_abundance.txt -ta $true_abund -rl $read_length -gl $genome_length -m $camisim_reads_mapping_path
        python $scripts_dir/species_metrics_eval.py -i - -t sylph -rt $read_type -s $samplesID -dt $data_type -pa sylph_abundance.txt -ta $true_abund -rl $read_length -gl $genome_length -m $camisim_reads_mapping_path -e 0
    elif [ $profile_level == "strain_level" ]; then
        python $scripts_dir/sylph_convert.py result.tsv $seq2tax strain
        python $scripts_dir/strain_evaluation.py sylph_abundance.txt sylph $data_type $true_abund $database_genomes_info > evaluation_report.txt
    fi    
