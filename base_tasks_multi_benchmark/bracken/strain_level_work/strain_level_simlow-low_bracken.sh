...
if [ ! -f bracken_query_report_species ]; then
    if [ $profile_level == "strain_level" ]; then
        /usr/bin/time -v -o query_time.log bracken -d $db -i $wd/kraken2/$profile_level/$dataset/$samplesID/kraken2_query_report -o bracken_query_report_species -r $read_length
    fi
fi
