...
read=/home/work/gyli/real_human_gut/ngs/SRR19064874_qc3_modified.fastq.gz
genome_length=-
...
if [ ! -f $strain_abundance ]; then
    ...
    if [ $need_create == "False" ]; then
        ...
        if [ $read == "None" ]; then
            /usr/bin/time -v -o create_db_time.log $pantax -f $database_genomes_info -s -p -r $read1 -r $read2 --create --mode 1 -A 95 -t $threads $zip_paras
        else
            /usr/bin/time -v -o create_db_time.log $pantax -f $database_genomes_info -s -p -r $read --create --mode 1 -A 95 -t $threads
        fi
