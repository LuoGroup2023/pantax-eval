...
generate_legend <- function(file_path, pos) {
  ...
  data <- data %>% mutate(Tools = recode(Tools, "PanTax(fast)" = "PanTax (fast)"))
...
generate_plot_time <- function(file_path, dataset_label, output_file_path) {
  ...
  data <- data[!grepl("clr|R9", data$sample_id, ignore.case = TRUE), ]
  data <- data %>% mutate(Tools = recode(Tools, "PanTax(fast)" = "PanTax (fast)"))
...
p_split <- print(p1_split)/print(p2_split)/print(p3_split)/legend + plot_layout(nrow=4,heights=c(5,5,5,1))
output_file_path <- "time_report/plots/all2"
ggsave(paste0(output_file_path, "_time_facet.pdf"), plot = p_split, dpi = 300, width = 10, height = 15)
